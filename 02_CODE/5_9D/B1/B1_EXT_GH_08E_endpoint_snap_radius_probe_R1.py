...
def helper_self_tests() -> None:
    ...
    fixture = (
        "graphhopper:\n"
        "  datareader.file: 'missing.pbf'\n"
        "  graph.location: 'graph-cache'\n"
        "  profiles: []\n"
        "server:\n"
        "  application_connectors: []\n"
    )
    patched, action = set_or_insert_graphhopper_yaml_int(
        fixture, "index.max_region_search", 64
    )
    if action != "INSERTED_IN_GRAPHHOPPER_SECTION":
        raise AssertionError("graphhopper config insertion action self-test")
    if "\n  index.max_region_search: 64\n" not in patched:
        raise AssertionError("graphhopper config indentation self-test")
    if re.search(r"(?m)^index\.max_region_search:", patched):
        raise AssertionError("graphhopper key leaked to YAML root")
    if patched.index("  index.max_region_search: 64") > patched.index("server:"):
        raise AssertionError("graphhopper key inserted outside graphhopper block")
...
def set_or_insert_graphhopper_yaml_int(
    ...
    """
    Set a GraphHopper engine property INSIDE the top-level ``graphhopper:``
    mapping used by graphhopper-web 11.

    The previous GH08E attempt incorrectly appended ``index.max_region_search``
    at YAML root level. In graphhopper-web 11 the engine properties belong
    under ``graphhopper:``, while root level is reserved for Dropwizard
    sections such as ``server:`` and ``logging:``.
    """
    lines = text.splitlines()

    # Active exact-key occurrences (comments excluded because lstrip starts '#').
    hits = [
    ...
    # Find exactly one root-level graphhopper mapping.
    gh_hits = [
        i for i, line in enumerate(lines)
        if line == "graphhopper:"
    ]
    ...
    # Determine the end of the graphhopper block: next nonblank,
    # noncomment root-level YAML key.
    gh_end = len(lines)
    ...
    if len(hits) == 1:
        ...
        if not (gh_i < i < gh_end):
            raise RuntimeError(
                f"Active '{key}:' exists outside the graphhopper section at line {i+1}"
            )
        indent = lines[i][: len(lines[i]) - len(lines[i].lstrip())]
        if len(indent) == 0:
            raise RuntimeError(
                f"Active '{key}:' is root-level; refusing unsafe rewrite"
            )
        old_line = lines[i].strip()
        lines[i] = f"{indent}{key}: {int(value)}"
        return "\n".join(lines) + "\n", f"REPLACED_IN_GRAPHHOPPER [{old_line}]"

    # Infer engine-property indentation from known mandatory engine keys.
    ...
    for ref_key in ("graph.location", "datareader.file"):
        for i in range(gh_i + 1, gh_end):
            if lines[i].lstrip().startswith(ref_key + ":"):
                indent = lines[i][: len(lines[i]) - len(lines[i].lstrip())]
                if indent:
                    reference_candidates.append((i, indent))
    if not reference_candidates:
        raise RuntimeError(
            "Could not infer indentation inside graphhopper section "
            "from graph.location/datareader.file"
        )

    # Require a single consistent indentation for these peer scalar keys.
    indents = {indent for _, indent in reference_candidates}
    if len(indents) != 1:
        raise RuntimeError(
            f"Inconsistent peer indentation inside graphhopper section: {indents}"
        )
    indent = next(iter(indents))

    # Insert immediately after graph.location when possible, otherwise after
    # the last known peer scalar. This keeps the key unambiguously inside
    # the graphhopper mapping and away from Dropwizard root-level sections.
    graph_loc_hits = [
        i for i in range(gh_i + 1, gh_end)
        if lines[i].lstrip().startswith("graph.location:")
    ]
    if len(graph_loc_hits) == 1:
        insert_at = graph_loc_hits[0] + 1
    else:
        insert_at = max(i for i, _ in reference_candidates) + 1
...
def make_server_config(
    ...
    text, region_action = set_or_insert_graphhopper_yaml_int(
...
def route_request(
    ...
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "Tesi-B1-EXT-GH08E-R1/1.0"},
    )
    try:
