...
# ======================================================================================
# B1-EXT-GH 04 v02 — GRAPHHOPPER PROFILE + IMPORT PREFLIGHT
#
# SAFE GATE:
...
def main() -> int:
    ...
    print("=" * 126)
    print("B1-EXT-GH 04 v02 — GRAPHHOPPER PROFILE + IMPORT PREFLIGHT")
    print("=" * 126)
    ...
    try:
        ...
        import_exposed = bool(re.search(r"(?im)^\s*import\b", help_text))
        check_exposed = bool(re.search(r"(?im)^\s*check\b", help_text))
        print(f"JAR --help return code = {help_proc.returncode}")
        print(f"import command exposed = {'YES' if import_exposed else 'NO'}")
        print(f"check command exposed  = {'YES' if check_exposed else 'NO'} (expected NO for GH 11)")

        if help_proc.returncode != 0 or not import_exposed:
            failures.append("GH_IMPORT_COMMAND_NOT_EXPOSED")
        if check_exposed:
            warnings.append("UNEXPECTED_CHECK_COMMAND_EXPOSED")
