...
8. preserves the endpoint-snap artifact byte-identically because GH10C proved
   identical start/end snaps for baseline and legal counterfactual;
9. materializes a new append-only v03 package plus an explicit override ledger.
...
def assert_same_snapped_endpoint(
    a: dict[str, Any],
    b: dict[str, Any],
    label: str,
    tol_deg: float = 1e-10,
...
def main() -> int:
    ...
    baseline_snaps = baseline.get("snapped_waypoints", [])
    counterfactual_snaps = counterfactual.get("snapped_waypoints", [])
    if len(baseline_snaps) != 2 or len(counterfactual_snaps) != 3:
        raise RuntimeError(
            "Unexpected GH10C snapped-waypoint cardinality: "
            f"baseline={len(baseline_snaps)} counterfactual={len(counterfactual_snaps)}"
        )
    assert_same_snapped_endpoint(
        baseline_snaps[0],
        counterfactual_snaps[0],
        "destination snapped endpoint",
    )
    assert_same_snapped_endpoint(
        baseline_snaps[-1],
        counterfactual_snaps[-1],
        "crossing snapped endpoint",
    )
    ...
    print("counterfactual endpoint snaps        = IDENTICAL_TO_BASELINE")
    ...
    for row in external_v03:
        ...
        # GH10C proved that the counterfactual start/end snapped waypoints
        # are identical to the frozen baseline. Therefore this override changes
        # routing impedance only; endpoint-snap provenance remains frozen v02.
        assert_close(
            parse_float(row["destination_snap_m"]),
            parse_float(snaps["destination"]),
            "frozen destination snap vs GH10C",
            DIST_TOL,
        )
        assert_close(
            parse_float(row["crossing_snap_m"]),
            parse_float(snaps["crossing"]),
            "frozen crossing snap vs GH10C",
            DIST_TOL,
        )
    ...
    changed_target_fields = {
        field
        for field in external_cols
        if str(old_target.get(field, "")) != str(new_target.get(field, ""))
    }
    expected_changed_target_fields = {
        "external_time_s",
        "external_distance_m",
        "external_weight",
    }
    if changed_target_fields != expected_changed_target_fields:
        raise RuntimeError(
            "External target changed unexpected fields: "
            f"{sorted(changed_target_fields)}"
        )
    ...
    print("external fields changed             = external_time_s|external_distance_m|external_weight")
    print("endpoint snap fields changed        = 0")
    ...
    # ------------------------------------------------------------------
    # G. Endpoint snap immutability audit
    # ------------------------------------------------------------------
    print()
    print("G. ENDPOINT SNAP IMMUTABILITY AUDIT")

    # Do NOT regenerate the endpoint-snap CSV here. GH08's helper returns a
    # normalized derivative representation whose string formatting/schema is
    # not guaranteed to be byte-comparable with the already-materialized v02
    # CSV. GH10C has already proved the physically relevant fact: adding the
    # roundabout waypoint changed neither the destination snap nor the crossing
    # snap. The correct v03 endpoint artifact is therefore an exact byte copy
    # of frozen v02.
    old_endpoint_lookup = {
        route_key(r): r for r in endpoint_old
    }
    if target_key not in old_endpoint_lookup:
        raise RuntimeError("Affected endpoint-snap key absent from frozen v02")

    endpoint_target = old_endpoint_lookup[target_key]
    if "destination_snap_m" not in endpoint_target or "crossing_snap_m" not in endpoint_target:
        raise RuntimeError(
            "Frozen endpoint-snap schema lacks destination_snap_m/crossing_snap_m"
        )
    assert_close(
        parse_float(endpoint_target["destination_snap_m"]),
        parse_float(snaps["destination"]),
        "endpoint artifact destination snap vs GH10C",
        DIST_TOL,
    )
    assert_close(
        parse_float(endpoint_target["crossing_snap_m"]),
        parse_float(snaps["crossing"]),
        "endpoint artifact crossing snap vs GH10C",
        DIST_TOL,
    )

    print(f"endpoint rows                       = {len(endpoint_old):,}")
    print("endpoint rows changed               = 0")
    print("endpoint artifact action            = BYTE_IDENTICAL_COPY_OF_V02")
    ...
    immutable_copy_pairs = [
        ...
        (gh08f_dir / ENDPOINT_V02, staging / ENDPOINT_V03),
    ...
    # endpoint_path was copied byte-identically from v02 above.
    ...
    summary = {
        ...
        "integrity": {
            ...
            "endpoint_rows_changed": 0,
            "endpoint_snap_artifact_preserved_byte_identical": True,
    ...
    print("ENDPOINT_ROWS_CHANGED = 0")
    print("ENDPOINT_SNAP_ARTIFACT = BYTE_IDENTICAL_TO_V02")
