...
def parse_member_tokens(members_text: str) -> list[dict[str, Any]]:
    """
    Parse GraphHopper's warning rendering robustly, including malformed OSM
    member roles.

    Examples observed in the frozen import log include:
      from way 123
      via node 456
      to way 789
      node 5612277457              # empty role
      via Tintoretto node 21493969 # unknown/free-text role

    Only the exact canonical OSM restriction roles ``from``, ``to`` and ``via``
    are promoted into ``role``. Any other prefix is preserved verbatim in
    ``role_raw`` and flagged as unknown, but is NOT silently coerced to ``via``.
    """
    out = []
    for ordinal, raw in enumerate(members_text.split(",")):
        # GraphHopper warning output can contain NBSP/non-ASCII whitespace.
        token = re.sub(r"\s+", " ", raw.replace("\u00a0", " ")).strip()
        if not token:
            continue

        # Parse from the RIGHT: member type + numeric OSM id are stable, while
        # the role prefix can be empty, canonical, or arbitrary malformed text.
        m = re.fullmatch(
            r"(?:(.*?)\s+)?(way|node|relation)\s+(\d+)",
            token,
        )
        if not m:
            raise RuntimeError(
                f"Unparsed restriction member token after whitespace "
                f"normalization: {token!r}"
            )

        role_raw = (m.group(1) or "").strip()
        role = role_raw if role_raw in {"from", "to", "via"} else ""
        out.append({
            "ordinal": ordinal,
            "role": role,
            "role_raw": role_raw,
...
def relation_structure(relation: dict[str, Any]) -> dict[str, Any]:
    ...
    empty_role = [
        m for m in members
        if m.get("role_status") == "EMPTY"
    ]
    unknown_role = [
        m for m in members
        if m.get("role_status") == "UNKNOWN"
    ]
    all_way_ids = [
        m["member_id"]
        for m in members
    ...
    return {
        ...
        "unknown_role_count": len(unknown_role),
        "unknown_roles": [m["role_raw"] for m in unknown_role],
        "unknown_role_members": [
            f"{m['role_raw']} {m['member_type']} {m['member_id']}"
            for m in unknown_role
        ],
...
def helper_self_tests() -> None:
    ...
    malformed = parse_member_tokens(
        "from way 15243283, via Tintoretto node 21493969, to way 45792487"
    )
    if malformed[1]["role"] != "":
        raise AssertionError("unknown role must not be coerced to canonical via")
    if malformed[1]["role_raw"] != "via Tintoretto":
        raise AssertionError("unknown role raw preservation self-test")
    if malformed[1]["role_status"] != "UNKNOWN":
        raise AssertionError("unknown role status self-test")

    malformed2 = parse_member_tokens(
        "from way 1333712237, Via Vittorio Piatti node 265738560, to way 24450022"
    )
    if malformed2[1]["role_raw"] != "Via Vittorio Piatti":
        raise AssertionError("multiword unknown role self-test")

    empty = parse_member_tokens(
        "via node 11942189612, from way 1287680379, to way 1287680380,\u00a0 relation 17649421"
    )
    if empty[-1]["role_status"] != "EMPTY" or empty[-1]["member_type"] != "relation":
        raise AssertionError("NBSP empty-role relation self-test")
...
def route_way_sequence_request(cache_row: dict[str, str]) -> dict[str, Any]:
    ...
    # IMPORTANT:
    # Use the exact path-detail request shape already proven by GH09 on all
    # 2,048 selected external legs. In GraphHopper this project observed that
    # ``calc_points=false`` can omit the path-details payload even when
    # ``details=osm_way_id`` is requested. GH09 succeeded with calc_points=true
    # plus edge_id/osm_way_id/distance details, so GH10A deliberately reuses
    # that exact proven contract instead of introducing another variant.
    params = [
    ...
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "Tesi-B1-EXT-GH10A-R2/1.0"},
    )
...
def main() -> int:
    ...
    def run_phase(
        ...
        try:
            ...
            # Real endpoint contract canary. This is intentionally executed
            # synchronously before the 2k-route bulk run. If GraphHopper does
            # not return the expected path-details payload, fail here after one
            # request rather than after launching the entire batch.
            canary_key = keys[0]
            canary_result = route_way_sequence_request(
                selected_meta[canary_key]["cache_row"]
            )
            canary_result["search_mode"] = (
                "FALLBACK64" if fallback64 else "DEFAULT"
            )
            canary_result["selected_b1_relations"] = parse_int(
                selected_meta[canary_key]["selected_row"]["selected_b1_relations"]
            )
            canary_result["Pendolari_raw_sum"] = parse_float(
                selected_meta[canary_key]["selected_row"]["Pendolari_raw_sum"]
            )
            canary_result["flow_daily_sum"] = parse_float(
                selected_meta[canary_key]["selected_row"]["flow_daily_sum"]
            )
            route_results[canary_key] = canary_result
            print(
                "PATH_DETAIL_CANARY             = PASS "
                f"edge={canary_result['edge_detail_count']} "
                f"way={canary_result['way_detail_count']} "
                f"distance={canary_result['distance_detail_count']}"
            )
            ...
            with ThreadPoolExecutor(
                ...
                for fut in as_completed(futures):
                    ...
                    if completed % 100 == 0 or completed == len(keys):
                        ...
                        processed_bulk = max(completed - 1, 1)
                        rate = processed_bulk / elapsed
                        eta = (
                            len(keys) - completed
                        ) / max(rate, 1e-9)
                        with PRINT_LOCK:
    ...
    for key in sorted(route_results):
        ...
        route_way_rows.append({
            ...
            "edge_detail_count": r["edge_detail_count"],
            "way_detail_count": r["way_detail_count"],
            "distance_detail_count": r["distance_detail_count"],
    ...
    for rid in sorted(relation_objects):
        ...
        relation_summary_rows.append({
            ...
            "unknown_role_count": s["unknown_role_count"],
            "unknown_roles": "|".join(s["unknown_roles"]),
            "unknown_role_members": "|".join(s["unknown_role_members"]),
    ...
    relation_fieldnames = [
        ...
        "unknown_role_count", "unknown_roles", "unknown_role_members",
    ...
    unknown_role_relations = sum(
        1 for s in structures.values()
        if s["unknown_role_count"] > 0
    )

    print(f"route-relation incidences       = {len(incidence_rows):,}")
    print(f"structurally incomplete rels    = {structurally_incomplete}")
    print(f"relations with unknown roles    = {unknown_role_relations}")
    ...
    summary = {
        ...
        "relations_with_unknown_roles": unknown_role_relations,
    ...
    print(
        f"RELATIONS_WITH_UNKNOWN_ROLES = "
        f"{unknown_role_relations}"
    )
    print(
