...
def route_detail_request(cache_row: dict[str, str]) -> dict[str, Any]:
    ...
    for item in edge_details:
        ...
        seg_len = polyline_length_m(seg_coords)
        if seg_len <= 0:
            raise RuntimeError(
                f"Non-positive geometry length for edge {edge_id}"
            )
        edges.append({
            "edge_id": edge_id,
...
USED_EXTERNAL_EDGES = {summary['used_subgraph']['used_edge_ids']}
...
SUM OF MAX OBSERVED ROUTED-GEOMETRY LENGTH PER UNIQUE GRAPHHOPPER EDGE_ID
...
def main() -> int:
        ...
        for e in edges:
            ...
            rec = edge_acc.setdefault(
                edge_id,
                {
                    ...
                    "osm_way_ids": set(),
                    "route_count": 0,
                    "max_observed_segment_length_m": 0.0,
                    "max_flow_daily_sum": 0.0,
                },
            )
            rec["osm_way_ids"].add(way_id)
            rec["max_observed_segment_length_m"] = max(
                float(rec["max_observed_segment_length_m"]),
                float(e["segment_length_m"]),
            )
            if edge_id not in route_seen_edges:
                rec["route_count"] += 1
                route_seen_edges.add(edge_id)
    ...
    used_edge_rows = [
        {
            "edge_id": edge_id,
            "osm_way_ids": "|".join(
                str(x) for x in sorted(rec["osm_way_ids"])
            ),
            "route_count": rec["route_count"],
            "max_observed_segment_length_m": rec["max_observed_segment_length_m"],
        }
        for edge_id, rec in sorted(edge_acc.items())
    ]
    used_node_rows = [
        rec
    ...
    used_edge_union_length_m = sum(
        float(r["max_observed_segment_length_m"])
    ...
    print(f"used GraphHopper edge ids      = {len(used_edge_rows):,}")
    ...
    summary = {
        ...
        "used_subgraph": {
            ...
            "used_edge_ids": len(used_edge_rows),
            ...
            "length_semantics": (
                "SUM_MAX_OBSERVED_ROUTED_GEOMETRY_LENGTH_PER_UNIQUE_GH_EDGE_ID"
            ),
        },
